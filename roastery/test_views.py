...
class ViewTests(TestCase):
    ...
    def test_origin_products_view(self):
        response = self.client.get(reverse('origin_products',
                                           args=['African']))
        self.assertEqual(response.status_code, 200)
    ...
    def test_purchase_form_view(self):
        response = self.client.get(reverse('purchase_form',
                                           args=[self.product.product_id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_product_view_get(self):
        ...
        self.assertTemplateUsed(response,
                                'roastery/product_confirm_delete.html')
        self.assertIn('product', response.context)
    ...
    def test_delete_product_view_post(self):
        response = self.client.post(reverse('delete_product',
                                    args=[self.product.product_id]))
        self.assertEqual(response.status_code, 302)
        self.assertFalse(Product.objects.filter(product_id=
                                                self.product.product_id).exists())
