...
class ProductViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.user.user_permissions.add(self.add_permission, self.change_permission, self.delete_permission)
    ...
    def test_create_product_view_get(self):
        ...
        self.assertRedirects(response, reverse('manage_products') + '?success_message=Product created successfully!')
    ...
    def test_update_product_view_post(self):
        ...
        self.assertTrue(Product.objects.filter(product_id=self.product.product_id).exists())
        response = self.client.post(reverse('update_product', kwargs={'product_id': self.product.product_id}), data=form_data)
        self.assertRedirects(response, reverse('manage_products') + '?success_message=Product updated successfully!')
    ...
    def test_delete_product_view_post(self):
        ...
        self.assertRedirects(response, f'{reverse("manage_products")}?success_message=Product deleted successfully!')

    def test_delete_product_view_without_login(self):
        self.client.logout()
        response = self.client.post(reverse('delete_product', args=[self.product.product_id]))
        self.assertRedirects(response, f'{reverse("login")}?next={reverse("delete_product", args=[self.product.product_id])}')
